...
test('NON-NEGOTIABLE: registryToSteps still excludes the workspace-only agents', () => {
  ...
  assert.equal(steps.length, 17, 'workspace-only agents are excluded from the step list'); // +1: enableClarifier, +1: shellGate
  assert.deepEqual(steps.map((s) => s.key), [
    ...
    'onboardingClarifier', 'enableClarifier', 'onboardingAnalyzer', 'onboardingTests', 'onboardingEvaluator', 'onboardingCanary', 'shellGate',
  ]);
...
test('AGENT_STEPS (derived from the registry) is byte-identical to registryToSteps and has 17 entries', () => {
  assert.equal(AGENT_STEPS.length, 17); // +1: enableClarifier (Enable app), +1: shellGate

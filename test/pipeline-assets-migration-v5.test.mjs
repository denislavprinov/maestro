// test/pipeline-assets-migration-v5.test.mjs
...
const homes = [];
beforeEach(async () => {
  const dir = await realpath(await mkdtemp(join(tmpdir(), 'maestro-assets-mig-v5-')));
  homes.push(dir);
  _resetForTests();
...
test('opening a user_version=4 DB forward-migrates to v5 (adds pipeline_assets)', () => {
  ...
  assert.equal(
    ...
    0, 'seed (v4) has no pipeline_assets table');
  seed.close();
  ...
  assert.equal(
    ...
    1, 'pipeline_assets table created by the v4->v5 migration');
});

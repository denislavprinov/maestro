// test/pipeline-assets-migration-v14.test.mjs
...
const homes = [];
beforeEach(async () => {
  const dir = await realpath(await mkdtemp(join(tmpdir(), 'maestro-assets-mig-v14-')));
  homes.push(dir);
  _resetForTests();
...
test('opening a user_version=13 DB forward-migrates to v14 (adds pipeline_assets)', () => {
  ...
  assert.equal(
    ...
    0, 'seed (v13) has no pipeline_assets table');
  seed.close();
  ...
  assert.equal(
    ...
    1, 'pipeline_assets table created by the v13->v14 migration');
});

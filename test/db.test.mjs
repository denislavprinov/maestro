...
test('migrate stamps user_version = 11', () => {
  const db = getDb();
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 11, 'schema version is 11 after migrate');
});
...
test('getDb() calls maybeMigrateFromFs(db) once after migrate()', () => {
  ...
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 11, 'migrate() ran before the hook');
...
test('getDb() first-launch is concurrency-safe across N processes (no lock/exists crash)', async () => {
  ...
  const db = getDb();
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 11, 'migrated to v11');
  assert.equal(

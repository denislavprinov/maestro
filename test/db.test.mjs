...
test('migrate stamps user_version = 5', () => {
  const db = getDb();
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 5, 'schema version is 5 after migrate');
});
...
test('getDb() calls maybeMigrateFromFs(db) once after migrate()', () => {
  ...
  // (user_version === 5 proves migrate() ran before the hook).
  ...
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 5, 'migrate() ran before the hook');
...
test('getDb() first-launch is concurrency-safe across N processes (no lock/exists crash)', async () => {
  ...
  const db = getDb();
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 5, 'migrated to v5');
  assert.equal(

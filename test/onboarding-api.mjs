...
//    The title path of suggestBranchName (worktree.mjs:91-94) slugifies the title
//    VERBATIM and does NOT strip stopwords (only keywordSlug(prompt) does), so "for"
//    is KEPT. pipeline.id is 8 lowercase hex (artifacts.mjs:643 randomBytes(4)) and
//    suggestBranchName slices it -> maestro/enable-project-for-ai-<8hex>.
test('done resolves a summary with the Enable branch', async () => {
  const { done } = await runOnboarding({ projectDir: freshRepo(), answers: ANSWERS, mock: true });
  const r = await done;
  assert.equal(r.status, 'done');
  assert.match(r.branch, /^maestro\/enable-project-for-ai-[0-9a-f]{8}$/); // title slug keeps "for"; id is 8 hex

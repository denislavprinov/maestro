...
test('exactly eight routed views', () => {
  assert.equal((html.match(/data-view/g) || []).length, 8);
});
test('eight views include projects + composer + the two workspace views', () => {
  for (const v of ['new', 'running', 'history', 'projects', 'composer', 'workspaces', 'workspace-create', 'settings'])
    assert.ok(html.includes(`data-view="${v}"`), `missing data-view=${v}`);
});
test('nav targets: the base views + projects + workspaces (workspace-create is NOT a nav target)', () => {
  for (const v of ['new', 'running', 'history', 'projects', 'composer', 'workspaces', 'settings'])

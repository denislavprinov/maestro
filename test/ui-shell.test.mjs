...
test('exactly five routed views', () => {
  assert.equal((html.match(/data-view/g) || []).length, 5);
});
test('five nav targets', () => {
  for (const v of ['new','running','history','composer','settings']) assert.ok(html.includes(`data-nav="${v}"`), `missing data-nav=${v}`);
});

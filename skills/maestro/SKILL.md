...
The orchestrator repo lives wherever it was installed. `<MAESTRO_REPO>` below is the absolute path of that repo (the directory containing `src/cli/maestro.mjs`). If you installed via `scripts/install.mjs`, the installer rewrites `<MAESTRO_REPO>` in this file to the real path automatically; otherwise substitute it yourself (or set an `MAESTRO_REPO` environment variable and use `"$MAESTRO_REPO"`).
...
```bash
node <MAESTRO_REPO>/src/cli/maestro.mjs --project "$PWD" --prompt "<args>"
```
...
```bash
node <MAESTRO_REPO>/src/cli/maestro.mjs \
...
```bash
node <MAESTRO_REPO>/src/cli/maestro.mjs --ui
```
...
```bash
node <MAESTRO_REPO>/scripts/install.mjs "<targetDir>"
```

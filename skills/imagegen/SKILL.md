...
When the requested image must contain a specific logo or wordmark, never let
the model draw it from a text description — it will mangle the lettering.
Instead, pass the real logo file (ask the user for it if not provided) as an
input image and instruct the model to place it faithfully:
...
Pick a logo colorway that contrasts with the background (light mark on dark
backgrounds, dark/colored mark on light ones).

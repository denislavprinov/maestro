...
1. **AI-readiness score /100** — weighted rubric:
   ...
   - multi-tool coverage — read the user's actual `multiToolTargets` **choice** (NOT the question's `default`; the choice is a compound label like `"AGENTS.md + .cursor/rules + Copilot instructions"`). Parse it into the set of requested targets and verify each requested file exists on disk: `AGENTS.md` → `AGENTS.md`; `.cursor/rules` → `.cursor/rules*` or `.cursorrules`; `Copilot` → `.github/copilot-instructions.md`. **Every requested target that is missing is a HARD failure** — emit a `critical` issue naming the missing file(s) so the `s_eval → s_infra` loop fires. **Parity, not bare presence: each requested file must also be SELF-SUFFICIENT** — open it and verify it carries (1) the verified build/test commands, (2) the architecture/conventions core, (3) a key-workflows section mirroring the feature skills. A pointer-only stub ("see CLAUDE.md") or a file missing any of the three counts as MISSING (same `critical`, name the file and what it lacks). The bar: a fresh Copilot or Codex session must land on guidance equivalent to what a fresh Claude session gets — score `multiTool` by the weakest requested tool, and do not score it 100 unless every requested target is present and self-sufficient. (When the choice is `none`, judge against exactly that.)
...
- **review**: the standard protocol review (md + json). JSON shape `{ "issues": [{ "severity", "title", "detail", "location" }], "summary", "score": <0-100> }`. Include `score` (this cycle's AI-readiness /100) — the review json is written per-cycle (`<key>-review-cycleN.json`), so it is the durable per-cycle score archive (the `readiness` card is a single canonical "latest" file that each cycle overwrites).
- **readiness**: write the report card markdown to the `readiness` md path AND the machine JSON to its json path — a CANONICAL "latest" card that every cycle OVERWRITES (no per-cycle suffix), so it always reflects the final cycle. JSON shape: `{ "score": <0-100>, "baselineScore": <0-100|null>, "delta": <signed int|null>, "dimensions": { "docs": n, "skillsAgents": n, "rules": n, "tests": n, "featureSkillCoverage": n, "realTests": n, "vendoring": n, "multiTool": n, "codeHealth": n }, "gaps": ["<unmet item>"] }`. Carry `baselineScore` straight from `graph.baselineReadiness.score` (the analyzer's pre-generation measurement of the repo as it started) and set `delta = score − baselineScore` (null when no baseline). State the baseline → final delta near the top of the markdown card too ("started at 28/100, finished at 93/100, +65") so the run's value is legible at a glance. `featureSkillCoverage` is soft (scored, never blocks); `realTests` is hard (blocks per the rule below). Use `null` for either when N/A (no `skillCandidates` / no `pureUnits`).
